...
def draw_gun():
    ...
    if mouse_pos[0]!=gun_point[0]: #for saving our butt when x2-x1 results to 0.
        slope = (mouse_pos[1] - gun_point[1]) / (mouse_pos[0] - gun_point[0]) #just calculating the slope (y2-y1)/(x2-x1)
    else:
        slope = -100000  #randomly assigning negative large value.
    angle = math.atan(slope)
    rotation = math.degrees(angle)
    if mouse_pos[0]< WIDTH/2:
        gun = pygame.transform.flip(guns[level - 1],True,False) #True is flipping in x drection and False is flipping in y direction.
        if mouse_pos[1] <= 600:
...
file.close()
best_freeplay = int(file_read[0])
best_ammo = int(file_read[1])
best_timed = int(file_read[2])
